...
const CSV_URL =
  "https://docs.google.com/sheets/d/e/2PACX-1vTPqiRFxht-An0D4qkvXOddlecVuv0LIE1gOEEq93MBwYuFgtaa3pfrvg67s0ZhXsEpvMxgaMz77zUn/pub?gid=1024027397&single=true&output=csv";
...
async function fetchAndInjectSVG(data) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        "Erro ao carregar o mapa SVG. Verifique o caminho 'imagens/map_brazil.svg'."
      );
    }
    ...
    if (svgElement) {
      ...
      mapContainer.innerHTML =
        "Erro: O arquivo SVG não contém um elemento <svg> válido.";
    }
  } catch (error) {
    ...
    mapContainer.innerHTML +=
      '<img src="https://placehold.co/400x400/005586/ffffff?text=Mapa+do+Brasil" style="width: 100%; border-radius: 8px; margin-top: 15px;">';
  }
}
...
async function loadAndProcessData() {
  try {
    ...
    console.log(
      "Dados Processados armazenados (Detalhado por Município):",
      DADOS_DETALHADOS_POR_MUNICIPIO
    );
