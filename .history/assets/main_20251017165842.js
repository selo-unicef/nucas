const CSV_URL =
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vTPqiRFxht-An0D4qkvXOddlecVuv0LIE1gOEEq93MBwYuFgtaa3pfrvg67s0ZhXsEpwMxgaMz77zUn/pub?gid=1024027397&single=true&output=csv";
  ...
  try {
    ...
    for (let i = 2; i < rows.length; i++) {
      ...
      if (columns.length >= 7 && status && status !== "---") {
        ...
        if (status === "✅ NUCA criado" && uf.length === 2) { // Garante que é uma UF válida (2 caracteres)
    ...
    console.log("Contagem de NUCAs por UF:", NUCA_COUNT_BY_UF);
    ...
    // 7. Geração do Mapa e Gráfico de Barras por UF (CHAMADA PRINCIPAL)
...
function getColorForValue(value, min, max) {
    ...
    // Usando as cores do tema do dashboard
    const colorMin = [171, 225, 250]; // ABE1FA (azul claro)
    const colorMax = [23, 128, 118];  // 178076 (verde azulado escuro - cor do NUCA criado)

    if (max <= min) {
        // Se todos os estados tiverem a mesma contagem (ou zero/vazio), usa uma cor base
        const r = Math.round((colorMin[0] + colorMax[0]) / 2);
        const g = Math.round((colorMin[1] + colorMax[1]) / 2);
        const b = Math.round((colorMin[2] + colorMax[2]) / 2);
        return `rgb(${r},${g},${b})`;
    }
...
function renderMap(svgElement, nucaDataByUF) {
    ...
    // Se não houver NUCAs criados em nenhum estado
    ...
    // Filtra para encontrar o min e max dos estados que TEM NUCAs (> 0)
    ...
    // Define o valor mínimo para o gradiente como 0, se o min for maior que 0
    // Isso garante que o estado com a menor contagem receba a cor mais clara do gradiente.
    ...
    svgElement.querySelectorAll('path[id^="BR"]').forEach(path => {
        ...
        let color;
        if (nucaCount === 0) {
            // Cor para estados sem NUCAs criados (cinza muito claro, quase o fundo)
            color = '#EFEFEF'; 
        } else {
...
async function carregarMapa(nucaDataByUF) {
    ...
    // Remove qualquer conteúdo prévio (como a tag <img> se ainda estiver lá)
    ...
    try {
        ...
        if (!response.ok) {
            ...
            mapContainer.innerHTML = '<p style="text-align:center; color:red; font-weight: bold;">Erro ao carregar o mapa SVG. Verifique o caminho.</p>';
...
function createBarChart(nucaDataByUF) {
    ...
    // Destrói o gráfico anterior se existir (para evitar problemas de re-renderização)
